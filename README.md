...
> 🔒 **This repository is a portfolio showcase.** It documents how this system works and how to set it up, but the working n8n workflow file (`.json`) is not included here — it's delivered directly to clients who purchase it via خمسات, مستقل, or my store. Interested? Reach out through one of those platforms. 👇
...
4. [📂 Part A — Google Drive & OAuth Setup](#part-a--️-google-drive--oauth-setup)
5. [🔑 Part B — Google Gemini API Key](#part-b--google-gemini-api-key)
6. [🗄️ Part C — Supabase Setup](#part-c-️-supabase-setup)
7. [🔌 Part D — Import & Connect in n8n](#part-d--import--connect-in-n8n)
8. [🚀 Part E — Activate & Test](#part-e--activate--test)
9. [🛠️ Troubleshooting](#9-️-troubleshooting)
10. [⚠️ Limitations & What's Not Included](#10-️-limitations--whats-not-included)
...
2. ✂️ Breaks it into small, meaningful chunks
...
📂 Google Drive (your documents)
        │
        ▼
⚙️  n8n (automation engine — watches folder, processes files)
        │
        ▼
🗄️  Supabase (database — stores document content + search index)
        │
        ▼
...
- ⚙️ Access to an n8n instance (self-hosted or n8n Cloud)
- 📦 The workflow file (`.json`) provided at purchase
- ⏱️ About 30–45 minutes for first-time setup

---

## Part A — 📂☁️ Google Drive & OAuth Setup

This system reads files from a Google Drive folder you control. To let n8n access it securely, you'll create your own free Google Cloud project — a one-time setup, and it means **you own this connection**, not a third party. 🔐
...
## Part C — 🗄️ Supabase Setup

Supabase is where your document content and search index live. No prior database experience needed — you'll paste one script and click Run. 🖱️
...
- 🏗️ **Creates the table** `company_documents` — like a spreadsheet with columns for the chunk's text, its metadata, and its embedding. This table does not exist beforehand; this script is what creates it.
- 🔍 Creates a search function named `match_documents` that the chatbot calls to find the most relevant chunks for any question (this exact name is required — it's hardcoded on the n8n side and can't be changed)

> ⚠️ **If asked about Row Level Security (RLS):** choose to enable it. This is the secure default — it locks the table so only your backend (via a special key) can access it, and it will not affect how your chatbot works.

> 🔄 **If you ever delete the table** (accidentally or on purpose), just re-run this exact same script in a fresh SQL Editor query — it will recreate the table and function from scratch. Note that deleting the table also deletes all previously uploaded document data; recreating it gives you an empty table, and documents will need to be re-added to the Drive folder to repopulate it.

<details>
<summary>📜 <strong>Fallback: Full Script (click to expand, in case the .sql file is unavailable)</strong></summary>
...
--    (this is what actually creates company_documents — it does not exist until this runs)
...
-- 3. Enable Row Level Security (safe default; service_role key used by
--    n8n bypasses this automatically, so it won't break the workflow)
alter table company_documents enable row level security;

-- 4. Similarity search function.
--    Must be named exactly "match_documents" -- this name is hardcoded
--    in the n8n Supabase Vector Store node and cannot be customized.
...
You'll create 3 credentials in n8n, each set up once but applied across multiple nodes automatically:
...
| 🗄️ Supabase API | Part C (Project URL + Secret key) | 2 nodes |

To create each: **Credentials → Add Credential**, search for the type, paste in your values, save.

### D3. Connect Every Node

Open the workflow canvas. Any node labeled **"REPLACE: ..."** 🏷️ needs your credential selected from its dropdown. Click each one, choose the credential you just created.
...
3. Wait about a minute ⏱️ (the system checks for new files every minute)
...
## 10. ⚠️ Limitations & What's Not Included
...
- 🗑️ **Deleting a file from Drive does not remove it from the knowledge base** — outdated information must currently be removed manually from the Supabase table, or as a future enhancement
...
- 🗄️ **Extracted text + search index**: stored in your own Supabase project (a database you control, not a shared/third-party database)
...
- [ ] 🏗️ Google Cloud project created, Drive API enabled, OAuth credentials created
- [ ] 🗄️ Supabase project created, setup script run successfully
